...
* Deflection and Resonance Filters: Calculates continuous deflection functions to verify angular deflection limits at bearings, and checks critical speeds (using Rayleigh's method) to avoid system resonance.
...
Note: Implementing a dynamic stopping condition requires careful nuance. A simple "stop if improvement < threshold" logic may terminate the algorithm prematurely if the population hits a temporary evolutionary plateau or if the elite seed remains unchanged for a few consecutive generations before a beneficial mutation occurs.
...
* Removing Intelligent Design: The current GA jump-starts the population by seeding three known, viable bloodlines. Future iterations will aim to make this designer oversight obsolete, relying entirely on raw random initialization and natural selection.
...
## Note on AI Usage
While all core physics equations, logic architecture, the GA algorithm, and math code were initially written by hand using class resources and standard engineering textbooks, I utilized AI assistance during the development of this project. AI was used strictly for mathematical validation, code correction, and learning extracurricular programming concepts (such as bloodline seeding Genetic Algorithms). Additionally, AI was used to help generate the repetitive boilerplate for print debugging and the final validation reporting, as that work was tedious to format by hand and did not provide additional personal learning value.
